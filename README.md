...
**Comparison against an outside tool** drove the current revision. A figr.design run on the same problem produced real TSX against a packaged Dopamine build, and beat ours on every axis that mattered: it re-voiced the page's own status-band construct for the intervention (a `sunrise-glow.99` tint with a `.95` border) instead of inventing a grey box with an accent stripe; it made the design mismatch visible with a countdown and a progress bar; and it modelled the assumption we had merely labelled — per-test `same-sample` vs `new-sample`, tube-level eligibility, report-delay propagation. Six harness changes came out of that read, the sharpest being that `SKILL.md`'s component list was stale and omitted `ActionCard`, whose own documentation names "test recommendation card" and "labs cross sell".
