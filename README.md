...
**Stage 2 — Compose.** Four phases against the live Dopamine MCP. The wireframe's component candidates are treated as a hypothesis, not a decision — every zone is re-resolved with `search_components` and `get_component_docs`, and composed patterns are checked first since a pattern beats assembling its parts. Every value resolves through the three-layer token hierarchy (base → semantic → component), including the Stage 1 type *roles*, which become real `--font-size-*` tokens here. Props come from the actual TypeScript contracts: inventing a prop or a union value is a defect, not a shortcut, and a zone that maps to no component is named as a system gap rather than approximated with CSS overrides. The accessibility guidelines run in full as the gate to Stage 3. Output: a composed surface with tokens resolved and a surface report.
...
**Source inspection is a mandatory gate.** When screenshots, screens, or a flow are supplied, every relevant frame must actually be opened and inspected, and a source-language extraction shown before construction — naming the sources and at least three markers that could only have come from them. "Preserve the product style" does not satisfy it, and an unreadable reference means asking for a usable one rather than silently falling back to the neutral system.
